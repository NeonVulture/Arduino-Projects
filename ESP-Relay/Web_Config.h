...
String projectDesc = 
  "<ul><li>Control an Relay remotely via a web server"
  "<li>Manually toggle the relay on and off."
  "<li>Set relay for timed operation based on pre-defined intervals."
  "<li>Send out an S.O.S distress signal.</ul>";
...
String processor(const String& var) {
  ...
  if(var == "RELAYBUTTONPLACEHOLDER") {
    String Relay_Button = "";
    String RelayOutputStateValue = OutputState();
  ...
  if(var == "TIMERBUTTONPLACEHOLDER") {
    String Timer_Button = "";
    String TimerOutputStateValue = OutputState();
    Timer_Button += "<h4>Timer: <span id=\"outputState3\"><span></h4><label class=\"switch\"><input type=\"checkbox\" onchange=\"toggleCheckbox3(this)\" id=\"output3\" " + TimerOutputStateValue + "><span class=\"slider\"></span></label>";
    return Timer_Button;
  }
 
  if(var == "SOSBUTTONPLACEHOLDER") {
    String SOS_Button =  "";
    String SOSOutputStateValue = OutputState();
...
void InitWebConfig() {
  ...
  server.on("/aboutme", HTTP_GET, [](AsyncWebServerRequest *request)
        ...
        <body style='font-size:1.1em;'>
          <h3>ESP32/8266 Relay About Page</h3>
